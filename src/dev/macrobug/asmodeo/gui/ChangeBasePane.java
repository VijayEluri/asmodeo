...
import dev.macrobug.asmodeo.gui.listener.Base;
import dev.macrobug.asmodeo.gui.listener.ChangeBase2;
...
public class ChangeBasePane extends JPanel{
	...
	public ChangeBasePane(MyF f,JLabel codici,JLabel numero,JFrame F,JRadioButtonMenuItem[] jrb){
		...
		n.setEditable(false);
		in.getDocument().addDocumentListener(new Base(in,out,n));
		can=new JButton("Annulla");
		start=new JButton("Pronto");
		can.addActionListener(new ChangeBase2(n,out,in,f));
		start.addActionListener(new ChangeBase2(start,n,out,f,codici,numero,F,jrb));
		indica.add(in);
		indica.add(n);
		o.add(indica);

...
public class Util implements Code{
	...
	public char leggi(String st,boolean bool){
		...
		for(i=st.length()-1,k=0,numero=0;i>=0;k++,i--){
			if(max.indexOf(st.charAt(i))!=-1){
				...
				numero=63;
				if(!bool){
				Dialogo d=new Dialogo("Errore","� stato inserito un carattere non valido");
				d.show();}
				throw new NumberFormatException();}}
			return (char)numero;}
	...
	/*Questo v� eliminato serve solo per mascherare gli errori*/
	public int leggiN(String st,boolean bool)throws NumberFormatException{
		...
		for(i=st.length()-1,k=0,numero=0;i>=0;k++,i--){
			if(max.indexOf(st.charAt(i))!=-1){
				...
				if(!bool){
				Dialogo d=new Dialogo("Errore","� stato inserito un carattere :"+st.charAt(i)+"\nnon valido");
				d.show();}
				throw new NumberFormatException();}
			}
		return numero;}

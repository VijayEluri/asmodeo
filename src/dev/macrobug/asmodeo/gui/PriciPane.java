...
import dev.macrobug.asmodeo.gui.listener.Bottoni;
import dev.macrobug.asmodeo.gui.listener.ChangeBase;
import dev.macrobug.asmodeo.gui.listener.Help;
import dev.macrobug.asmodeo.gui.listener.Menu;
import dev.macrobug.asmodeo.gui.listener.Princi;
import dev.macrobug.asmodeo.gui.listener.Scr;
import dev.macrobug.asmodeo.gui.listener.SelezionaCodice;
import dev.macrobug.asmodeo.gui.listener.Stampa;
...
public class PriciPane extends JPanel implements Externalizable,ActionListener{
	...
	//Inizio menu
		...
		co.addActionListener(new Help(3,f));
		M.add(warning);
		M.add(about);
		M.add(shor);
		M.add(co);
		...
		jrbmi[4].addActionListener(new Menu(codice,numero));
		B.add(jrbmi[0]);
		B.add(jrbmi[1]);
		B.add(jrbmi[2]);
		B.add(jrbmi[3]);
		...
		salva.addActionListener(new Stampa(out,this,codice,numero,true));
		...
		Bottoni rad=new Bottoni(comp,in,out,b,false,codice,numero,f);
		Bottoni radt=new Bottoni(comp,in,out,b,true,codice,numero,f);
	...
	public void readExternal(ObjectInput entro)throws IOException,ClassNotFoundException{
        ...
        if(entro.readBoolean()){
        	b.addActionListener(new Princi(comp,b,in,out,codice,numero));
        	scr.setSelected(true);}
        else{
        	b.addActionListener(new Scr(comp,in,out,codice,numero,f));

...
public final class EdiTable extends JComponent implements ChangeCellListener {

  private static final long serialVersionUID = 1L;
  private final ArrayList<SimpleTextField> lines;
  private final int column, charCell;

  public void setText(String s) {
    String temp;
    int j;
    for (j = 0; j < lines.size() && s.length() > 0; j++) {
      temp = s.substring(0, charCell);
      lines.get(j).setText(temp);
      s = s.substring(charCell);
    }
    lines.get(j).requestFocusInWindow();
  }
  ...
  void fireCellEmptied(ChangeCellEvent event) {
  ...
  void fireCellFulled(ChangeCellEvent event) {
    ChangeCellListener array[] = listenerList.getListeners(ChangeCellListener.class);
  ...
  void fireRowEmptied(ChangeCellEvent event) {
    ChangeCellListener array[] = listenerList.getListeners(ChangeCellListener.class);
    for (ChangeCellListener ccl : array) {
  ...
  void fireRowFulled(ChangeCellEvent event) {
    ChangeCellListener array[] = listenerList.getListeners(ChangeCellListener.class);
    for (ChangeCellListener ccl : array) {
      ccl.rowFulled(event);
    }
  ...
  void addChangeCellListener(ChangeCellListener ccl) {
    listenerList.add(ChangeCellListener.class, ccl);
  }

  void removeChangeCellListener(ChangeCellListener ccl) {
    listenerList.remove(ChangeCellListener.class, ccl);
  }

  ChangeCellListener[] getChangeCellListeners() {
    return listenerList.getListeners(ChangeCellListener.class);
  }
  ...
  private class SimpleTextField extends JTextField implements DocumentListener {
    ...
    public SimpleTextField(int c) {
      super(c);
      getDocument().addDocumentListener(this);
      setMaximumSize(new Dimension(getFont().getSize() * c, 2 * getFont().getSize()));
    }
